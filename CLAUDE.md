...
- [x] 解卦传文层(已发 v1.11.0)——工作台断卦卡(RuleCard)的每句所占经文配上十翼对应传:卦辞配彖(「彖传释卦」)、爻辞配小象(「小象释爻」)、用九用六配各自小象;参考折叠的卦辞展开后亦带彖。数据全来自 hexagrams.json 既有 tuan/xiaoxiang/extra.use.xiaoxiang,引擎 engine/divination.js 的 getDivinationResult 给每个 text 挂 commentary 字段,**只释卦象不断吉凶(守 §9 禁算命口吻)**;配 divination.test.js 9 例覆盖 n=0..6 全断法
