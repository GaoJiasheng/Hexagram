...
- [x] 十六期:儒典四书(docs/design-v16.md,已发 v1.17.0)——内容期,标准对齐道藏。**通用 corpus 基建一次落地**:`scripts/fetch-corpus.mjs <key>`+`scripts/corpus/<key>.config.mjs`(每源页一章,剔标题/章号/页脚/`<ref>`校勘/多变体`-{zh-hans}-`,补异体字 愼/擧);前端 `src/features/reader/corpus.js`(glob 限 {fo,ru},道藏不牵入)+ `CorpusTextPage/ReadPage/SinglePage`+通用 `YanyiBlock`+ App.jsx `/fo|/ru` 路由;check-data 泛化 `checkReadingCorpus`+`checkCorpusAnchors`(道藏输出不变)。**四书全译注延**:论语 507 段/20 篇延伸/960 注疏、孟子 745 段/14 卷/955 注疏、大学 11 段、中庸 34 段;底本论语孟子取维基文库通行子页、大学中庸取《礼记》古本(章句本缺经/带注)。论语孟子由**并发 workflow**(每篇/卷 译+校双代理)产出后程序化装配(term 须原文精确子串、note≤40、段数对齐,违规即弃)。**改儒内容照 v16 分书风格(朱熹《四书章句集注》为主流),逐批 check-data 过才 commit**
- [ ] 十七期(**已落稿待开工**):释典三经(docs/design-v17.md,十七期→v1.18.0)。复用 v16 通用 corpus 路。**佛站守「研习不宣化、不下吉凶/果报断语」(v17 §0 铁律)**。底本待定坑:心经(玄奘)主页 djvu 转嵌、金刚经(罗什)32 分作 `====X分====` 标题需按标题切章、坛经宗宝本此版无品标题需另觅底本。**开工前先读 v17 批次表与分书风格,逐批 check-data 过才 commit**
