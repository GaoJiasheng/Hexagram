...
- [ ] 十六/十七期(**已落稿待开工**):儒典四书(docs/design-v16.md,十六期→v1.17.0)、释典三经(docs/design-v17.md,十七期→v1.18.0)。内容期,标准对齐道藏(原文管线+译文+锚定注疏+每章/篇延伸)。**儒先佛后**;批次 1 一次性**泛化通用 corpus 装载层 + 通用阅读器三页 + check-data 泛化**(道藏保持原样不动,佛/儒走新通用路),其后只填译注延。佛站守「研习不宣化、不下吉凶/果报断语」(v17 §0 铁律)。底本:儒取维基文库朱熹章句本经文、佛取罗什/玄奘译本与坛经宗宝本。**开工前先读对应稿的批次表与分书风格,逐批 check-data 过才 commit**
