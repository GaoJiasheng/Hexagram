...
**docs/yijing-design.md 是页面、交互、视觉、推演规则、数据结构的唯一规格。** 实现任何页面前先读对应章节,不要自行发明视觉风格或交互。实现顺序按设计稿 §10 的里程碑 M1 → M2 → M3,每个里程碑完成后逐条核对其验收清单。

## 当前状态(随进度更新此节)

- [x] M0 脚手架:路由占位壳、设计 tokens(src/index.css)、favicon、数据管线
- [ ] M1 读:HexagramFigure 组件、六十四卦总览、卦详情页、导航/主题/译文开关
- [ ] M2 推:推演引擎(纯函数 + 单测)、推演工作台
- [ ] M3 通:经传阅读、八卦基础、我的、全局搜索、今日一卦
- [ ] 内容:63 卦译文待补(见下方「译文工序」),xugua/zagua 按卦引文、palace(八宫)字段待填
