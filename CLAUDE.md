...
- [x] 十七期:释典三经(docs/design-v17.md,已发 v1.18.0)——复用 v16 通用 corpus 路,守 v17 §0 铁律(研习不宣化、不下吉凶/果报断语)。管线加 `splitHeadings`(金刚经 32 分按 `====X分====` 切章、去『』夹注、跳「正文/外部链接」)+ `PIN_TITLE_RE`(剔品题行)。**三经全译注延**:心经 10 段/32 注疏、金刚经 253 段/32 分延伸/337 注疏、坛经 555 段/10 品延伸/519 注疏(并发 workflow 产出+程序化装配:term 须原文子串、note≤40、段数对齐;超长 note 句读处截断、重叠锚点去重)。**底本**:心经玄奘略本维基文库仅 djvu 转嵌不可洁取,改用法成广本(authorNote 标明,**待 owner 定夺是否换**);金刚经罗什/江味农校定本;坛经宗宝本 `六祖壇經/{10品}` 子页。**改佛内容守 §0 铁律,逐批 check-data 过才 commit**
