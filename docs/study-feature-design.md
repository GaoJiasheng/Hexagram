...
> **已实现。做新书请看作业标准 [books-production-standard.md](books-production-standard.md)(SOP,反映落地状态)。** 本稿为原始产品/UI 设计(路由写作 `/study`,实现已改为 `/books`;文章"每小节一篇"已收敛为"每一级章一篇";脑图为 4 级句子树;文章一律先弹抽屉、⤢ 再进整页;书封为 `BookCover` 生成式母题)。
>
