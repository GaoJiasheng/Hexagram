...
| 层 | 决定 | 现状 |
|---|---|---|
| **典籍** | **封关**,不再收新书 —— 「再加价值也不大了」 | 67 部 |
| **争鸣辩题** | **封关**,不再加新辩 | 88 辩 |
| **难经白话** | **不做**(中医组,81 难。原文/译注/延伸齐全,只是不铺白话) | 0/81 |

同类早先已定的:传习录条级普通档不做。

> **唯一还开着的口子:唐诗宋词。** owner 提过「唯一剩下的就是唐诗宋词」但未拍板做不做。
> 这会是**新的一组**(现十组皆为经子,诗词属集部),不是往现有组里塞书。
> 粗估:唐诗三百首 + 宋词三百首 ≈ 600 首,若照诗经的诗级粒度铺白话即 600 篇。
> **要做先定三件**:① 收哪个选本(《唐诗三百首》蘅塘退士本?全唐诗选?)
> ② 分组归属(独立新组 vs 挂儒典)③ 粒度(诗级 or 组级)。
