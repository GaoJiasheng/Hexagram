...
争鸣 84 辩 · 书级导读 62 篇 · 家级导读 0 篇 · iOS build 46 / 版本 1.0.0。
...
**顺序**:① **墨家样板** → owner 验收 → ② A 类其余四组 → ③ B/C 两类。
**厚度**:儒/道 9000–10000 · 佛 8000–10000 · 法兵医谋 7000–8000 · 墨/纵横/心学 6000–8000。

## 2.2 观书 · 下一批选什么 ◆ 待 owner 定方向

[books-roadmap.md](./books-roadmap.md) 的三簇 14 本(人类学/补薄标签/尼采)**已全部收官**,现 112 本。
可让我按现有标签分布提候选。作业标准见 [books-production-standard.md](./books-production-standard.md)。
...
# 四、零散增强(做不做由 owner 定,都不阻塞)

- **名句集** —— 对标「今日一卦」:论语「学而时习之」、金刚经「应无所住」…
  分享卡 `QuoteCard` 已有,**缺的是集子本身**。投入最小、露出最高。
- **书页面小传加厚** —— `authorNote` 62/62 全有但**中位仅 106 字**,只答了「谁写的」。
  可补三问:这本在讲什么 · 从哪一篇入手 · 哪几篇最容易误读。
  **优先级低于家级导读**——书级导读已经把这些讲了,小传只是入口摘要。
- **章内 TOC 跳转** —— 长章拆页 + 段级续读已上线(37 个 ≥100 段的长章都能定位),
  剩的只是章内目录跳转(现 TOC 只到篇/卷/品级)。
- **孟子章号体例** ◆ —— 现按 14 卷,可加「梁惠王上·N」式章号便于引用。纯体例小事。
- **逐页动态 OG meta** —— 现为站点级静态 OG;逐页需构建期预渲染或 `/api/og` 对爬虫 UA 返回。
- **长短经「七雄略/三国权」两篇** ◆ —— 整卷即一篇(46820 字),内部无任何标记、无法自动切。
  三条路:(a) 维持卷级一篇概览(现状)(b) 人工按史事分段手写切分表 (c) 机械按段区间切(不推荐)。
