...
- [ ] **① 译注** —— 白话直译 + 字词注疏。唐诗 320 / 宋词 283 / 元曲(随选目)。
      ⚠️ **词牌曲牌必须注明「调名,与内容无关」**(《木兰花》不写木兰花)——初学者最常见的误解。
- [ ] **② 家级导读「一家之来路」** —— 三组各一篇。**属 SOP 的 C 类:不是流派,是体裁**,
      题目要问「这一体怎么成的、怎么变的」,写成流派谱系即错。
- [ ] **③ 每首白话深读** —— 粒度三组不同:唐诗走「组-序」细粒度键(同诗经),
      宋词元曲走纯章号。普通档 3000–4500 字 · 3–4 图。
- [ ] **④ 元曲扩充选目**(前置于①③)—— 现仅 12 首。加曲目**必须先探测页面存在**再写页名,
      标题体例繁简/`·`与空格/括号消歧极不统一,凭印象写必然抓空;当心转写壳(自身 0 段)。
      查无原文者一律不收,**严禁凭记忆补曲文**。
