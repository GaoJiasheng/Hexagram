...
## 2.2 诗词曲三组 —— 框架已成,三层内容待做

**唯一作业标准:[poetry-production-standard.md](./poetry-production-standard.md)**(动手前必须读完)。
换 session 接手照那份做即可,不照那份做的产出不收。

**框架已完成(2026-08-03,零内容)**:三组站点注册(观唐 `#9c4a3f` / 观宋 `#4a7a86` / 观元 `#4d5f6b`)、
三条管线跑通、书目 texts.json、路由/书架/`/me`/白话整页/搜索/书目索引/篇幅档/内容分片全部接入,
浏览器验过渲染。**原文已在库**:

| 组 | 底本 | 已抓 |
|---|---|---|
| 唐诗 tangshi | 《唐诗三百首》蘅塘退士 1763 | 7 类 · 320 首 · 2257 段 |
| 宋词 songci | 《宋词三百首》朱孝臧 1924 | 283 首 · 2021 段 |
| 元曲 yuanqu | **无传世选本,本站选目** | 12 首 · 43 段 |

### 现状核对(2026-08-05 复核,以磁盘为准)

**在库的只有原文一层**,三组的 `status` 全是 `partial`:

| 层 | 唐诗 | 宋词 | 元曲 |
|---|---|---|---|
| 原文 | ✅ 7 类 320 首 2257 段 | ✅ 283 首 2021 段 | ⚠️ 仅 12 首 43 段 |
| **译文**(`translation`) | ❌ 全 `null` | ❌ 全 `null` | ❌ 全 `null` |
| **注疏** `zhushi-anchored/` | ❌ 无目录 | ❌ 无目录 | ❌ 无目录 |
| **每首延伸** `yanyi.json` | ❌ | ❌ | ❌ |
| **白话深读** `baihua/` | ❌ | ❌ | ❌ |
| **书级导读** `daodu/` | ❌ | ❌ | ❌ |
| **家级导读** `school.json` | ❌ | ❌ | ❌ |

> 📌 **顺带修正一处口径**:书级导读那轮收官时说的「62/62 全齐」,统计脚本只枚举了
> 经子十组。**全站实为 65 部书,导读 62/65**,缺的正是这三部。家级导读同理 **10/13**。

### 待做(共 6 项,标了先后)

- [ ] **⓪ 接管线**(前置于①③,纯代码)—— `gen-zhuzi-wf.mjs`(产译注延)与
      `gen-baihua-wf.mjs`(产白话)**都还不认得这三个 corpus**,`grep tangshi` 零命中。
      两处各加分支 + 各自的 styleRule/红线(见 SOP §2 §4 §6),否则无法起批。
      另需确认 `check-data` 的 `chapterText` 对唐诗「组-序」键走的是《诗题》识别那一支。
- [ ] **① 元曲扩充选目**(前置于②④)—— 现仅 12 首,少得不成一组。
      可选池:`scripts/corpus/_poetry-lists.json` 的 `yuanqu.authors` —— 65 位作者 354 个页名,
      **但张养浩、乔吉、徐再思、贯云石、睢景臣五位名家不在池中**(维基文库枚举本身不全),
      要补他们得逐页探测。加曲目**必须先探测页面存在**再写页名(标题体例繁简/`·`与空格/
      括号消歧极不统一,凭印象写必然抓空;当心转写壳自身 0 段)。查无原文者一律不收,
      **严禁凭记忆补曲文**。
- [ ] **② 译注**(第一层)—— 白话直译 + 字词注疏。唐诗 320 / 宋词 283 / 元曲(随选目)。
      ⚠️ **词牌曲牌必须注明「调名,与内容无关」**(《木兰花》不写木兰花)——初学者最常见的误解。
- [ ] **③ 每首延伸**(第二层)—— 一首一则,讲创作本事/意象源流/声律安排/后世化用。
- [ ] **④ 每首白话深读**(第三层)—— 粒度三组不同:唐诗走「组-序」细粒度键(同诗经),
      宋词元曲走纯章号。普通档 3000–4500 字 · 3–4 图。**量最大的一项**(600+ 篇)。
- [ ] **⑤ 书级导读 3 篇 + 家级导读 3 篇** —— 书级照
      [daodu-production-standard.md](./daodu-production-standard.md);家级**属 SOP 的 C 类:
      不是流派,是体裁**,题目要问「这一体怎么成的、怎么变的」,写成流派谱系即错。
      这两项**不依赖译注**,可以先做、也最快见效(每组一两篇就让空站有内容)。

**顺序建议**:⓪ → ⑤(先让三站有导读、不空)→ ①(元曲补够)→ ②③(译注延一轮出)→ ④(白话最后)。
三组之间可并行,组内必须串行(站内老规矩:两大 workflow 齐发必撞限流)。
