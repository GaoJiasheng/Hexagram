...
> ~~**唯一还开着的口子:唐诗宋词。**~~ **owner 2026-08-03 已拍板做,且加了元曲**;
> 框架已搭完(见 §2.2),三层内容待做。以下原始估算留档:
...
另:若日后要加心性方向的概念聚类(把庄子《养生主》、坛经无念、信心铭「唯嫌拣择」、
清静经、内经「恬惔虚无」串成一条线),`src/data/concepts.json` 现有 5 个聚类里**一个心性的都没有**
——那件事成本极小(改一个 JSON,义理互见/术语表/专题三处自动同步),收益可能比再收书还大。
