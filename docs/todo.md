...
### 2.1a 站内数据小瑕疵(诗经批次中发现,待清)

- **《邶风·匏有苦叶》「軓」字站内自相矛盾**:`zhushi-anchored/shijing.json` 注疏作
  「车厢前端横木」,而同段译文作「车轴头」(旧本或作「軌」,两字形训释不同)。
  白话 `3-9` 已如实并存标疑、未替站内拍板;真源在
  `scripts/authored/ru-translations.json`,改需两处同改(真源+生成物)。
- **《邶风·北风》正文里混入韵脚标注**:段 87/90/93 的 `original` 末尾带
  `【韵：雱行】`/`【韵：霏归】`/`【韵：乌车】`——源页的编者标注被当正文收了。
  译文正确地未译,注疏还专加两条解释「非诗句正文」(等于拿注疏给数据缺陷打补丁)。
  全书仅此一首、共 3 处。**修法**:`fetch-corpus.mjs` 的 cleanLine 剔 `【…】`
  (标注是行尾内联、不独立成段,故剔掉不改段数,译文/注疏位置索引安全),重跑后
  连带撤掉注疏那两条补丁 + 白话 `3-16` 里解释标注的那个 mute 框。
  **须等诗经这批白话全做完再动**——重跑会改 classics 文件,在跑的 agent 正读它。
- **《卫风·硕人》「美目盻兮」疑为源页讹字**(待 owner 定夺,**不擅改**):站内作
  「盻」(xì,恨视),而通行《毛诗》作「盼」(pàn,目黑白分明),站内《论语·八佾》
  子夏引此句亦作「盼」——**同一站内两书用字不一**。已查 `scripts/.cache/wikisource.json`
  的 `詩經/碩人` 源页:**源页本身即作「盻」,管线忠实照录、非抓取错误**。
  按硬规矩原文不手改;若要改,属「底本择字」决策,须 owner 拍板后在管线加
  逐字校正表(而非直接编辑生成物)。硕人不在白话精选列,暂不影响产出。
- 机器产出的译注中此类「注疏与译文不同源致互相打架」可能不止一处,
  值得单开一轮扫查(比对同段 zhushi.term 释义与 translation 用词)。
