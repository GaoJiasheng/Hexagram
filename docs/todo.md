...
| 白话 | **2012 章** | **0 坏引文**,每章有图。⚠️ **新加的 4 部还没铺白话**,见 §2.3 |
...
### ⚠️ 唯一欠的:这 4 部的白话(17 + 2 = 19 章)

其余 70 部都有白话,这 4 部没有,是**目前内容侧唯一的形态缺口**。
补的话照 `gen-baihua-wf.mjs` 的 `dao` / `songci` 分支;
道三书守上面那把尺子(不演工法、不下成仙断语)。
