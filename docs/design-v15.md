...
- 三域名互不可见,owner 自己跨组只能手敲 URL。为省事另设一个**无任何站内链接指向**的隐藏门户:`MASTER_PORTAL_PATH = '/hexagram'`(可改;取项目代号,别人难联想到是总入口)。
...
- 域名着陆重定向对 `/hexagram` **豁免**(否则在 con/bud 域名上会被弹回本组首页),门户得以在任一域名打开。
...
- [ ] 隐藏门户 /hexagram 列三组卡片、dev 相对 / 生产绝对跨域、着陆重定向豁免
