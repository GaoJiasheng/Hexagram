...
- [x] ~~在 App 里确认能看到评论、举报/屏蔽点得动~~
- [x] ~~录屏~~ —— **已完成**,成品在桌面 `Hexa-Review-Recording-Final.mp4`
      (2:45 · 9.8 MB · 720×1564 H.264)
- [x] ~~确认演示账号可登录~~ —— 2026-08-16 实测 `appreview@hexa.gavin.pub` 登录正常
- [x] ~~密码~~ —— ASC 的「登录信息」本来就填好了,备注里改为指向该字段,不重复写
- [x] ~~保留录屏素材~~ —— **长期有效:别删 `/dao/daodejing/1` 那条评论和 `test@gmail.com`**
- [x] ~~Notes + 解决中心回复~~ —— 2026-08-16 均已填并提交
- [x] ~~仍用 build 50~~ —— 确认无需重新构建

**✅ 2026-08-16 21:15 已重交**(ASC API 核实:`appStoreState` 与
`reviewSubmissions.state` 均为 `WAITING_FOR_REVIEW`)。此为第三次提交。
...
桌面 **`Hexa-Review-Recording-Final.mp4`** · 2:45 · **9.8 MB** · 720×1564 H.264 ·
由两段真机录像合成(主流程 2:09 + 举报屏蔽补录 0:37),原始两段合计 122 MB。

**覆盖对照**:

| Apple 要求 | 片中位置 |
|---|---|
| 从启动 App 开始 | 开头(TestFlight 启动 → 每日一辩弹窗) |
| 核心功能 | 读经 / 译文 / 注疏 / 白话总纲 / 金句卡 |
| Account **registration** | ~0:25(含一次「该邮箱已注册」的错误处理) |
| **login** | ~1:36 |
| Account **deletion** | ~1:55(警告文案 → 抄邮箱确认 → 完成 → 退回门户) |
| User-generated content | ~1:20 评论区两条 + 只读提示 |
| content **reporting** | ~2:33 理由选择器 → 提交 → 「收到举报,我们会尽快处理」 |
| content **blocking** | ~2:40 「屏蔽中…」→ **该评论消失** → 「已屏蔽…可在设置解除」 |
| 敏感权限提示 | ~0:16 蜂窝数据弹窗 |

> ⚠️ **第一版录屏漏了举报与屏蔽** —— 理由选择器展开后被取消,屏蔽根本没点。
> **查生产库的 `comment_reports` / `user_blocks` 两张表坐实的**(都是空的),
> 不是靠看画面猜。补录后两张表各有一条记录(20:40:37 / 20:40:42),才算数。
> **教训:演示「机制」要看到落库或明确的完成反馈,按钮出现在画面里不等于演示了。**
