...
## 3.5 模拟器走查发现的问题(2026-08-17)

走查范围:首页四屏 → 门户 → 道藏书架 → 清静经(单页短经)→ 段操作 → 金句卡。
**渲染全部正常** —— 中文衬线、朱印徽章、组色、工具条、底栏、段操作按钮位置
(窄屏在段落下方右对齐)都与 iOS 一致。字体没问题是因为 **Noto Serif SC 自托管**,
294 个 woff2 随包发,不依赖系统字体。

发现两处:

### ① 🖼 金句卡的「下载图片」在原生壳里**静默失效**(要修)

实测:点了之后**日志无任何动静,Downloads 与 Pictures 都没有新文件**。
病因在 `QuoteCard.jsx:119`:走的是 `document.createElement('a')` + blob URL 触发下载,
而 **Capacitor 的 WebView 不处理这种下载**。

⚠️ **iOS 大概率同病**(WKWebView 同样不接 `<a download>` 的 blob),但没验过 ——
1.32.0 正在审核,不宜为此重新打包验证,等这轮过了再说。

这是今天修 Turnstile 那两次的同一个病:**界面在骗人** —— 按钮在、点了没反应、
也不报错。修法二选一:① 用 `@capacitor/filesystem` 写文件 + `@capacitor/share` 分享;
② 原生端直接把按钮换成诚实的说明(卡片下方本来就有「或直接截图本卡分享」)。

### ② 安卓没配 App Links(可选)

`am start -a VIEW -d "https://hexa.gavin.pub/ru/lunyu/1"` **落回首页,不认路径**。
即「分享一条链接 → 在 App 里打开对应章节」这条路安卓上不存在。
要做需加 intent-filter + 服务端 `assetlinks.json`。**不影响「可安装 APK」,优先级低。**
