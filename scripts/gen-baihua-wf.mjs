...
// ── 各组红线(design-v22 §3.6)──
const MOULUE_REAL_BOOKS = new Set(['changduanjing', 'caigentan', 'weiluyehua', 'xiaochuangyouji'])
// 观数(命理学,2026-09-19 立项)。铁律文本原样使用(owner 派工原文,常量名 TIELU_MINGLI),
// 与 gen-zhuzi-wf.mjs 里的同名常量文字一致(两脚本各自独立、不互相 import,保持同步靠人读)。
...
const RED = {
  ...
  // 谋略组含两类:5 部托名伪书(伪书批判)+ 4 部真书(长短经/菜根谭/围炉夜话/小窗幽记,思想史/处世研习,不作「伪书」框注)
...
const BOOK_STYLE = {
  ...
  // 修心三种(2026-08-08):道组红线在这三部比丹经更贴身 —— 丹经的隐语读者本就看不懂、
  // 不会照做;这三部句句像「可以照着试试」,故要单独把尺子写死。
  'dao/zuowanglun': [
    '【本书特有】《坐忘论》是唐代司马承祯讲「怎么把心收住」的七阶次第(信敬/断缘/收心/简事/真观/泰定/得道)。',
    '- **不演工法**:「学道之初,要须安坐,收心离境」这类入手指示,讲它是这一系统的主张即可,',
    '  **怎么坐、坐多久、如何离境一律不写,也不建议读者依此行事**。',
    '- 「坐忘」二字出自《庄子·大宗师》,但**司马承祯把庄子一句话的境界改成了一道可循的楼梯**',
    '  ——这个改造是全书最值得讲的地方;两书文字互有异文(离形去知/去智等),如实点出。',
    '- 「得道」章的形神合一、尸解等,是这部书自身的主张与许诺:**不下断语,也不反过来嘲弄**。',
    '- 《真观》里的业报、厌斥女色诸段,标为唐代道书吸纳佛教业报说的实例与中古出家语境,如实存之,不据以论断祸福。',
    '- **不比附现代心理技术**(不讲成正念/冥想/减压),不许诺任何效用。',
  ].join('\n'),
  'dao/tianyinzi': [
    '【本书特有】《天隐子》核心是「渐门」——修真达性不能顿悟,须循斋戒/安处/存想/坐忘/神解五级而进。',
    '- **作者如实存疑**:旧题司马承祯撰,一说他只作序,历来有异说,不替它拍板。',
    '- **《斋戒》《安处》两章是全书最像养生手册的地方**(节食调中、摩擦畅外、南向而坐东首而寝):',
    '  一律用「其说以为…」的间接体作学说陈述,**几时吃、怎么坐、屋子朝哪个方向一律不演**,',
    '  并明写不构成饮食起居的指导。顺带可点出它自己反对绝食求长生,是难得的节制之语。',
    '- 「存想」是上清派存思术之名:止于「谓存思身中之神与形,本书以收心复性释之」,**不写存思什么、怎么存**;',
    '  可讲这是唐代道教由「存思」向「心性」偏移的显例。',
    '- 「渐门」与禅宗顿渐之争恰成对照(站内佛组有《坛经》与「顿与渐」一辩),可点出这层对读,**但不作优劣判断**。',
  ].join('\n'),
  'dao/dingguanjing': [
    '【本书特有】《洞玄灵宝定观经》是唐代论「定」的短经,托言灵宝天尊告左玄真人,撰人不详(如实标)。',
    '- 全经眼目是那组对举:「唯灭动心,不灭照心;但凝空心,不凝住心」——讲透它。',
    '- **「若束心太急,又则成病,气发狂颠」值得专门点出**:一部教人修定的经,自己先说了用力过猛会出事。',
    '  但这一句**不涉任何医学判断或健康提示**,只作经文自身的节制之语来读。',
    '- 「得道七候」直到「延数万岁,名曰仙人」:通篇冠「其说以为」作品位名目的陈述,**不下长生成仙断语,也不嘲弄**;',
    '  并可点出七候在经文里的真正功能是一道**防伪条款**(经文紧接着否定「身无一候而自云慧觉、又称成道者」)。',
    '- 定慧对举、「慧而不用,实智若愚」与佛家止观、《坛经》定慧品互见,作**三教交涉的实例陈述**,不作高下判断。',
    '- **不比附现代心理技术**(不讲成正念/冥想/减压),不许诺任何效用。',
  ].join('\n'),
  'mingli/yuanhai': [
    '【本书特有】《渊海子平》多歌诀赋体(四言/七言韵语夹散论),白话时把韵语按义群拆开讲,',
    '**不勉强凑成顺口的白话韵文**,讲透字面义与命理逻辑即可。十神(比肩/劫财/食神/伤官等)、',
    '十二长生(长生/沐浴/冠带/临官/帝旺/衰/病/死/墓/绝/胎/养)等术语首见须讲清含义,后文再出可简引。',
  ].join('\n'),
  'mingli/zhenquan': [
    '【本书特有】《子平真诠》为论说体,设问自答、层层递进(「何谓成」「何谓败」…),',
    '白话时把这层论证结构讲清楚,不要拆散成孤立要点。格局/用神/相神/月令等术语全篇统一译法与解释,',
    '不同章节不得随文换用不同说法。书中数十处命造实例(如「甲午、乙酉、丙戌、丁亥,某参政命」)属沈孝瞻',
    '原文举证,照实讲解;不得引入徐乐吾等民国以后人的评注或例证。',
  ].join('\n'),
  'mingli/ditiansui': [
    '【本书特有】《滴天髓阐微》逐章分四类段落:①原文纲领(简短韵语)②原注(旧题刘基注)',
    '③任氏曰(任铁樵阐发)④命例——命例段已由管线结构化合并(kind:"mingli",original 为四柱与大运',
    '干支以空格相连,pillars/dayun 为结构化字段),**白话讲某个命例时,直接引用该段的四柱/大运干支',
    '(可用 quote 原样呈现),把紧随其后任氏的分析文字讲透即可,不要把干支串本身当成需要「翻译」的散文**。',
    '全书命例逾五百处,白话不必逐例讲,挑该章最能说明论点的一两个命例深入讲、其余点到为止。',
  ].join('\n'),
  'mingli/qiongtong': [
    '【本书特有】《穷通宝鉴》以「五行总论」开篇,其后十干各章内含「三春甲木总论」「三春甲木」等',
    '原书自身的小节题行(白话讲解时可当小标题用,不必逐字翻译这几个字本身)。',
    '核心是**调候**:「先用某,次用某」的取用次序须原样讲清先后(先取谁调候、次取谁辅佐),',
    '不可颠倒或只讲结论不讲次序;十干喜某忌某的判断照原书语气讲,不夸大为「必定」之类原文没有的断语。',
  ].join('\n'),
}
