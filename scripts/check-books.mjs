...
function checkArticle(file, o, { isOverview }) {
  ...
  for (const [i, svg] of figs.entries()) {
    ...
    //
    // 2026-08-21 又栽了一次(同一个教训):还有三类 `#xxx` 根本不是颜色 ——
    // ① `url(#f1a)` 箭头/渐变的 ID 引用 ② `id="f1a"` 定义本身
    // ③ `var(--paper,#fff)` 里的**兜底值**(这是正确写法,不是写死)。
    // 当时 14 条告警**全是**这三类,一条真问题都没有。先摘掉再判。
    const colorish = svg
      .replace(/url\(\s*#[\w-]+\s*\)/g, '')
      .replace(/\bvar\(\s*--[\w-]+\s*,[^)]*\)/g, '')
      .replace(/\b(?:id|xlink:href|href)\s*=\s*(['"])[^'"]*\1/g, '')
    if (/(?<!&)#(?:[0-9a-fA-F]{3}|[0-9a-fA-F]{6})\b/.test(colorish)) {
